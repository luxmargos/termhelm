...
describe('CLI argument parsing', () => {
  ...
  it('parses managed config mode', () => {
    expect(parseTerminalWindowsCliArgs(['managed', '--config', 'terminal-windows.json'])).toEqual({
      mode: 'managed',
      help: false,
      configPath: 'terminal-windows.json'
    });
  ...
  it.each(['package.json', 'test/__terminal-windows-missing-cwd__'])(
  ...
  it('rejects managed identity flags in plain or config mode', () => {
    ...
    expect(() => parseTerminalWindowsCliArgs([
      'managed', '--config', 'terminal-windows.json', '--label', 'dev'
  ...
  it('provides help text', () => {
    ...
    expect(helpText()).toContain('terminal-windows managed --label <label>');
